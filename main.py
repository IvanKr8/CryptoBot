...
BINANCE_WS_URL = "wss://stream.binance.com:9443/ws/solusdt@bookTicker"

bot = Bot(token=BOT_TOKEN)

# Скользящее среднее ордеров для SOL
...
async def notify_not_found(symbols_checked):
    msg = f"Пока крупных ордеров не найдено.\nПроверял токены: {', '.join(symbols_checked)}"
    await bot.send_message(chat_id=CHAT_ID, text=msg)
    ...
    async with websockets.connect(BINANCE_WS_URL) as ws:
        while True:
            try:
                ...
                print(json.dumps(data, indent=2))  # лог в консоль
                ...
                if asyncio.get_event_loop().time() - last_large_order_time >= NOT_FOUND_TIMEOUT:
                    await notify_not_found(["SOLUSDT"])
                    last_large_order_time = asyncio.get_event_loop().time()
